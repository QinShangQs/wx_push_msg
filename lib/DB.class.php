...
class DB {
    ...
    private function connect() {
        ...
        if (!$this->link) {
            ...
            Log::_write($this->log_task, "数据库连接错误：");
            Log::_write($this->log_task, '错误编号' . mysql_errno());
            Log::_write($this->log_task, '错误内容' . mysql_error());
            exit();
        }
    }
    ...
    public function reconnect() {
        if (!mysql_ping($this->link)) {
            ...
            $this->setCharset();
            $this->setDbname();
            Log::_write($this->log_task, "重建数据库连接");
        }
    }
    ...
    private function db_query($sql) {
        ...
        if (!$res) {
            // 结果出错了
            // 暴力处理，如果是真实线上项目（生产环境）必须写入到日志文件
            Log::_write($this->log_task, "语句出现错误：");
            Log::_write($this->log_task, $sql);
            Log::_write($this->log_task, '错误编号' . mysql_errno());
            Log::_write($this->log_task, '错误内容' . mysql_error());

            if (mysql_errno() == 2006) {//MySQL server has gone away
                $this->reconnect();
            }
            //exit ();
        }
        Log::_write($this->log_task, $sql);
        Log::_write($this->log_task, 'sql result is ' . $res);
        // 没有错误
        return $res;
    }
